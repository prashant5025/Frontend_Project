...
const Sidebar = () => {
  ...
  return (
    <section className="flex gap-6">
      <div className={`bg-[#0e0e0e] min-h-screen ${open ? 'w-72' : 'w-16'} duration-200 text-gray-100 px-4`}>
        ...
        <div className="mt-4 flex flex-col gap-4 relative">
          {menus?.map((menu, i) => (
            <Link
              to={menu?.link}
              key={i}
              className={`${menu?.margin && 'mt-5'} group flex items-center text-lg gap-3.5 font-medium p-2 hover:bg-gray-800  rounded-md`}>
              <div>
